...
/**
 * Format expiry date in a user-friendly way showing days until expiration
 * Returns object with text, color class, and urgency flag
 */
export const formatExpiryDate = (expiryDateStr) => {
  if (!expiryDateStr) {
    return { text: 'No date', color: 'text-gray-500', urgent: false };
  }
  ...
  if (diffDays < 0) {
    ...
    return { text: 'Expires tomorrow', color: 'text-orange-600', urgent: true };
  } else if (diffDays <= 3) {
    return { text: `Expires in ${diffDays} days`, color: 'text-orange-500', urgent: true };
  } else if (diffDays <= 7) {
    return { text: `Expires in ${diffDays} days`, color: 'text-yellow-600', urgent: false };
  } else {
    // Show day of week + date for items expiring later
    const options = { weekday: 'short', month: 'short', day: 'numeric' };
    return {
      text: `Expires ${expiryDate.toLocaleDateString('en-US', options)}`,
